...
character(len=40) :: mod = "SOC_initialization" ! This module's name.
public SOC_initialize_sponges
...
!  call get_param(PF, mod, "SOC_TNUDG", TNUDG, 'Nudging time scale for sponge layers (days)', &
...
!                 default=.false., do_not_log=.true.)
  ...
  if (associated(CSp)) call MOM_error(FATAL, &
          "SOC_initialize_sponges called with an associated control structure.")
  if (associated(ACSp)) call MOM_error(FATAL, &
          "SOC_initialize_sponges called with an associated ALE-sponge control structure.")
...
!  enddo ; enddo
  ...
  call get_param(PF, mod, "SOC_SPONGE_FILE", state_file, &
...
end subroutine SOC_initialize_sponges

end module SOC_initialization
